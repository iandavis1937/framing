...
// Ref: https://www.bennadel.com/blog/1504-ask-ben-parsing-csv-strings-with-javascript-exec-regular-expression-command.htm
  ...
  const input = fetch("http://127.0.0.1:5500/study2.csv");
		...
		.then(csvText => {
		  const fileData_raw = parseCSV(csvText, ",");
			const data = fileData_raw.filter(
				entry => entry.open_response && entry.open_response !== 'NA'
			);
		  console.log(data);
		  resolve(data);
		})
...
getData(input)
	.then(data => {
		// You can use the `data` variable here or pass it to other functions
