        ...
        reqSocket.once("message", function(reply) {
            ...
            response_arr.map((response, index) => {
                ...
                const OPINION_FRAME_REGEXES = [
                    {op_label: "op1", op_regex: new RegExp(`(?:\\bi\\b(?: )?(?:\\b\\w+\\b)?(?: )?(?:do|dont|don't|do not|can|can not|cant|can't|cannot|also|really|very much)?(?: )?(?:\\b\\w+\\b)? \\b(?:agree|believe|beleive|beleieve|consider|disagree|hope|feel|felt|think|thought|see|saw|question|questioned|suppose|am\\b \\bunsure|share(?: )?(?:\\b\\w+\\b)?(?: )?(?:\\b\\w+\\b)? (?:views|beliefs|opinions|perspectives|standpoints)|have(?: )?(?:\\b\\w+\\b)?(?: )?(?:\\b\\w+\\b)? (?:views|beliefs|opinions|perspectives|standpoints))\\b(?:.{0,20}))`, 'gmi')},
                    {op_label: "op2", op_regex: new RegExp(`(?:(?:\\bi'm\\b|\\bi\\b \\bam\\b)(?: )?(?:really|very much|not|not really|not very much)? [a-z]+ to \\b(?:agree|believe|beleive|beleieve|consider|disagree|hope|feel|felt|think|thought|see|saw|question|questioned|suppose|share(?: )?(?:\\b\\w+\\b)?(?: )?(?:\\b\\w+\\b)? (?:views|beliefs|opinions|perspectives|standpoints))\\b(?:.{0,20}))`, 'gmi')},
...
app.post('/csv', upload.single('csvFile'), async (req, res) => {  
    ...
    outputFilePath = 'output8_2.1_b1.csv' 
