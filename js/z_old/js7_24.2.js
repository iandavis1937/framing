    ...
    return new Promise((resolve, reject) => {
        ...
        reqSocket.once("message", function(reply) {
            ...
            response_arr.map((response, index) => {
                ...
                const AMBIGUOUS_WORDS_REGEX = new RegExp(`(?:\\b(?:seem|seems|seemed|prefer|prefers|preferred|might)\\b)`, 'gmi')
                ...
                const OPINION_FRAME_REGEXES = [
                    {op_label: "op1", op_regex: new RegExp(`(?:\\bi\\b(?: )?(?:\\b\\w+\\b)?(?: )?(?:do|dont|don't|do not|can|can not|cant|can't|cannot|also|really|very much)?(?: )?(?:\\b\\w+\\b)? \\b(?:agree|believe|beleive|beleieve|consider|disagree|hope|feel|felt|think|thought|suppose|am\\b \\bunsure|share(?: )?(?:\\b\\w+\\b)?(?: )?(?:\\b\\w+\\b)? (?:views|beliefs|opinions|perspectives|standpoints)|have(?: )?(?:\\b\\w+\\b)?(?: )?(?:\\b\\w+\\b)? (?:views|beliefs|opinions|perspectives|standpoints))\\b(?:.{0,20}))`, 'gmi')},
                    ...
                    {op_label: "op3", op_regex: new RegExp(`(?:\\b(?:in\\b my opinion|from my perspective|\\bin\\b my view|from my view|from my standpoint|from my viewpoint|from where I stand|\\bfor\\b \\bme|\\bto\\b \\bme|my views|my understanding)\\b(?:.{0,20}))`, 'gmi')},
                ...
                const NHS_REGEXES = [
                    {nhs_label: "nhs1", nhs_regex: new RegExp(`(?:\\bi\\b(?: )?(?:\\b\\w+\\b)?(?: )?(?:do|dont|don't|do not|can|can not|cant|can't|cannot|also|really|very much)?(?: )?(?:\\b\\w+\\b)? \\b(?:find|oppose|support|favor|lie\\b \\bwith|side\\b \\bwith)\\b(?:.{0,20}))`, 'gmi')},
                ...
                const FACT_FRAME_REGEXES = [
                    {f_label: "fp1", f_regex: new RegExp(`(?:\\b[tT]he\\b \\b[A-Z][a-z]+\\b \\b(?:can't|cannot|demonstrates|demonstrate|is|isn't|are|aren't|was|wasn't|were|weren't|does|doesn't|did|didn't|has|hasn't|had|hadn't|will|won't|would|wouldn't|should|shouldn't|needs|need)\\b(?:.{0,20}))`, 'gm')}, // deprecate?
                    {f_label: "fp2", f_regex: new RegExp(`(?:(?:\\b[A-Z][a-z]+\\b|\\b[a-z]+\\b) \\b(?:can't|cannot|demonstrates|demonstrate|is|isn't|are|aren't|was|wasn't|were|weren't|does|doesn't|did|didn't|has|hasn't|had|hadn't|will|won't|would|wouldn't|should|shouldn't|needs|need)\\b(?:.{0,20}))`, 'gm')},
                    {f_label: "fp3", f_regex: new RegExp(`(?:\\b[tT]he\\b [A-Z][a-z]+(?:\'s)? \\b[a-z]+\\b \\b(?:can't|cannot|demonstrates|demonstrate|is|isn't|are|aren't|was|wasn't|were|weren't|does|doesn't|did|didn't|has|hasn't|had|hadn't|will|won't|would|wouldn't|should|shouldn't|needs|need)\\b(?:.{0,20}))`, 'gm')}, // deprecate?
                    {f_label: "fp4", f_regex: new RegExp(`(?:X  (?:\\b[Hh][Ee]\\b|\\b[Ss][Hh][Ee]\\b|\\b[Ii][Tt]\\b|\\b[Tt][Hh][Ee][Yy]\\b) \\b(?:can't|cannot|demonstrates|demonstrate|is|isn't|are|aren't|was|wasn't|were|weren't|does|doesn't|did|didn't|has|hasn't|had|hadn't|will|won't|would|wouldn't|should|shouldn't|needs|need)\\b(?:.{0,20}))`, 'gm')}, /// deprecate?
                    {f_label: "fp5", f_regex: new RegExp(`(?:(?:\\b[Hh][Ee]\\b|\\b[Ss][Hh][Ee]\\b|\\b[Ii][Tt]\\b|\\b[Tt][Hh][Ee][Yy]\\b) \\b(?:can't|cannot|demonstrates|demonstrate|is|isn't|are|aren't|was|wasn't|were|weren't|does|doesn't|did|didn't|has|hasn't|had|hadn't|will|won't|would|wouldn't|should|shouldn't|needs|need)\\b(?:.{0,20}))`, 'gm')},
                    {f_label: "fp6", f_regex: new RegExp(`(?:\\b(?:[Cc][Aa]ndidate|[Cc][Ll]inton|[Dd][Oo]nald|gop|GOP|GOp|[Hh][Ii]llary|[Hh][Ii]lary|[Tt][Rr]ump|[Tt][Rr]um)\\b \\b(?:can't|cannot|demonstrates|demonstrate|is|isn't|are|aren't|was|wasn't|were|weren't|does|doesn't|did|didn't|has|hasn't|had|hadn't|will|won't|would|wouldn't|should|shouldn't|needs|need)\\b(?:.{0,20}))`, 'gm')},
...
app.post('/csv', upload.single('csvFile'), async (req, res) => {  
    ...
    outputFilePath = 'output7_24.2_br_repro.csv' 
