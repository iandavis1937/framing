...
class ClauseSegmentation:
    ...
    def fill_gaps(self, split_indices):
        filled_gaps = []
        for i in range(len(split_indices) - 1):
            # add the current range
            filled_gaps.append(split_indices[i])
            # if there's a gap to the next range
            if split_indices[i][1] + 2 < split_indices[i+1][0]:
                # add a range to fill the gap
                filled_gaps.append((split_indices[i][1] + 1, split_indices[i+1][0] - 1))
        # add the last range
        filled_gaps.append(split_indices[-1])
        return filled_gaps
    
    def remove_overlaps(self, split_indices):
        # Sort the tuples based on the start of the range
        split_indices.sort(key=lambda x: x[0])
        reduced = []
        current_start, current_end = split_indices[0]
        for i in range(1, len(split_indices)):
            # If the current range overlaps with the next one
            if current_end > split_indices[i][0]:
                # Update the end of the current range if the next one is wider
                current_end = max(current_end, split_indices[i][1])
            else:
                # If the current range does not overlap with the next one, save it and move on to the next one
                reduced.append((current_start, current_end))
                current_start, current_end = split_indices[i]

        reduced.append((current_start, current_end)) # append the last range
        return reduced
    ...
    def benepar_split(self, doc: Doc) -> list[tuple]:
        ...
        for sentence in doc.sents:
            ...
            split_indices += sent_split_indices
...
#test_str =
# """I think this is a benepar slash test and this could be a second (S split) .
# Just in case, here's number three which is a great opportunity to "add a fourth"(WHNP split)!"
# I believe this is a fifth test and 'here' is a sixth test which permits a seventh test as well(S then WHNP split).
# Here is an eighth test(no split). Additionally, here is an unnecessary comma laden ninth test, and this is a tenth test which has an eleventh test, and I think there is even a twelfth test as well, this is a thirteenth test.
# That said, I think this is a fourteenth test."""
...
test_str = """  I think this is a benepar\ test and this could be a       second (S split)  .
                ...
                Additionally, here is an unnecessary-comma-laden ninth test, and this is a tenth test which has an eleventh test, and I think there is even a twelfth test as well, this is a thirteenth test.
                That said, I think this is a fourteenth test."""
                

# Temp Test
test_str = """Trump is speaking what is on his mind without decorating or "filtering" his words or promises which makes me feel comfortable voting for him and gives me the impression that he is honest. Hillary chooses her each and every word when she talks which not makes me feel that she means exactly/and or feels what she is saying!"""
