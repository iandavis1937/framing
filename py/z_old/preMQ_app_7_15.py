...
import json
import requests
print("FastAPI and web framework mods imported... ", time.time() - start_time)
...
# Temp Test
# test_str = """ And, finally, here is an unnecessary-comma-laden ninth test, and this is a tenth test which has an eleventh test, and I think there is even a twelfth test as well."""
...
class Item(BaseModel):
    ...
    participant_input: Optional[str] = None

app = FastAPI()

origins = [
    "http://localhost",
    # add any other origins that might need to connect
]
...
@app.post('/py_output')
async def sendData(item: Item):
    print("FastAPI sending data endpoint reached...")
    print(f"Received data: {item}")
    print(f"Keys: {item.dict().keys()}")
    if item.participant_input or item.open_response:
        data_string = item.participant_input or item.open_response
        data_string = convert_number(data_string)
        data_string = convert_punctuation(data_string)
    ...
    if item.part_id:
        return_data["part_id"] = item.part_id

    response = JSONResponse(content=return_data, status_code=201)
