...
app.post('/node_to_flask', async (req, res) => {
	console.log("Sending data to Flask endpoint reached...");
	...
	// Log the data
		...
		try {
			// Get data sent from the client (Qualtrics)
			let participant_input = req.body.participant_input;
			// Send to Flask
			let flask_output = await axios.post('http://127.0.0.1:5000/flask_output', {participant_input: participant_input});
			for (let key in flask_output) {
				console.log(`${key}: ${flask_output[key]}`);
					}
			console.log(flask_output.data)
